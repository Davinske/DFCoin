...
#if defined(NDEBUG)
# error "davcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xbf560809e5342a9bebcff62fdba5b51c303eb416eb3476a89f5f2269f5318e3f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // davcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "davcoin Signed Message:\n";
...
{
    // davcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 25 * COIN; //Recompensa al minar un bloque

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 420000); // davcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // davcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // davcoin: 2.5 minutes per block
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // davcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // davcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x4e55149a5c374fca9df2d594b08b2ed4b0a47c5fa464b4fdc057971497149094");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Madrid TIME 22/05/2021 - Empire State Building does it's 50 birthday";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("046854326544b695ba275af6fbdb9f9bcedb8e58a7a2f14bb6a6592d94b3e54bf6fae15fb7970ffa17fc7fc17448308d709972b05dd859b196b97f562c33b407d1") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1621712480;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084724279;

        if (fTestNet)
        {
            block.nTime    = 1621712430;
            block.nNonce   = 386823104 ;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xa27d2065dcfcaf5aaad7ae407d7a9f0dcba68044c063b2acc709db5bba040d51"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc9, 0xb3, 0xdc }; // davcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("davcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("davcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static davcoinMiner(CWallet *pwallet)
{
    printf("davcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("davcoin-miner");
    ...
    try { loop {
        ...
        printf("Running davcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("davcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&davcoinMiner, pwallet));
}
